...
function accountFromSocket(socket) {
  ...
  return { name: socket.user.name || socket.user.notify || null, number: phoneFromJid(socket.user.id || '') || null };
}
...
function canonicalPairingPhone(value = '') {
  let phone = String(value || '').replace(/\D/g, '');
  if (phone.length === 10 || phone.length === 11) phone = `55${phone}`;
  if (phone.length < 10 || phone.length > 15) throw new Error('Informe um número do WhatsApp válido com DDD e país.');
  return phone;
}
...
export function createWhatsAppManager({
  ...
  function setStatus(patch) { status = { ...status, ...patch }; }
  function getStatus() { return structuredClone(status); }
  function markSeen(id) {
    if (!id) return true;
    if (seenIds.has(id)) return false;
    seenIds.add(id); seenQueue.push(id);
    while (seenQueue.length > maxSeenMessageIds) { const oldest = seenQueue.shift(); if (oldest) seenIds.delete(oldest); }
    return true;
  }
  function clearReconnect() {
    if (!reconnectTimer) return;
    clearSchedule(reconnectTimer); reconnectTimer = null;
  }
  function scheduleReconnectOnce() {
    if (manualDisconnect || reconnectTimer) return;
    reconnectTimer = schedule(() => {
      reconnectTimer = null;
      connect().catch((error) => setStatus({ status: 'error', error: error instanceof Error ? error.message : 'Falha ao reconectar WhatsApp' }));
    }, reconnectDelayMs);
  }
  function isAllowedDevMessage(message) {
    ...
    const matches = [primaryJid, alternateJid].filter(Boolean).some((jid) => canonicalDevPhone(jid) === allowedPhone);
  ...
  function normalizeDevOutboundRecipient(phoneOrJid) {
    ...
    if (canonicalDevPhone(jid) !== allowedPhone) throw new Error('Modo DEV: destinatário fora da allowlist do WhatsApp.');
    return `${allowedPhone}@s.whatsapp.net`;
  }

  async function performConnect() {
    manualDisconnect = false; clearReconnect();
    ...
    try {
      ...
      activeSocket.ev.on('connection.update', async (update = {}) => {
        ...
        if (connection === 'close') {
          ...
          setStatus({ status: 'disconnected', qrDataUrl: null, pairingCode: null, account: null, error: loggedOut ? 'Sessão do WhatsApp encerrada.' : null });
      ...
      activeSocket.ev.on('messages.upsert', async ({ messages, type } = {}) => {
        ...
        for (const message of messages ?? []) {
          ...
          if (!classifyInboundJid(jid).supported || !isAllowedDevMessage(message) || !markSeen(message?.key?.id)) continue;
      ...
      socket = null;
      setStatus({ status: 'error', qrDataUrl: null, pairingCode: null, account: null, error: error instanceof Error ? error.message : 'Falha ao conectar WhatsApp' });
      throw error;
    }
  }

  function connect() {
    if (socket && ['connecting', 'qr', 'pairing', 'authenticated', 'connected'].includes(status.status)) return Promise.resolve(getStatus());
  ...
  async function requestPairingCode(phone) {
    const normalizedPhone = canonicalPairingPhone(phone);
    if (!socket) await connect();
    if (!socket?.requestPairingCode) throw new Error('Pareamento por código não disponível nesta sessão do WhatsApp.');
    const pairingCode = await socket.requestPairingCode(normalizedPhone);
    if (!pairingCode) throw new Error('O WhatsApp não retornou um código de pareamento.');
    setStatus({ status: 'pairing', pairingCode: String(pairingCode), account: null, error: null });
    return getStatus();
  }

  async function disconnect() {
    manualDisconnect = true; clearReconnect();
    const activeSocket = socket; socket = null;
    if (activeSocket?.end) await Promise.resolve(activeSocket.end(new Error('PrismaStore desconectado')));
  ...
  async function sendMedia(phoneOrJid, source) {
    ...
    if (source?.base64) return socket.sendMessage(jid, { image: Buffer.from(source.base64, 'base64'), mimetype: source.mimeType || 'image/png', fileName: source.filename || 'imagem.png' });
    throw new Error('Mídia do WhatsApp inválida.');
  }

  return { connect, disconnect, requestPairingCode, getStatus, sendText, sendMedia };
}
