...
export function createDeliveryConfirmationService({
  ...
  function exportDossier(orderId) {
    ...
    const confirmation = order.deliveryConfirmation && typeof order.deliveryConfirmation === 'object'
      ? structuredClone(order.deliveryConfirmation)
      : {};
    const journey = order.deliveryJourney && typeof order.deliveryJourney === 'object'
