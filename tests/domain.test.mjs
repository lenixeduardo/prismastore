...
import { isLowStock, isNewAddress, calculateCart, confirmPayment, buildMonthlyReport } from '../src/domain.js';
...
test('marks an address as new when it differs from every historical address', () => {
  const customer = { addresses: [{ street: 'Rua Aurora', number: '120', zip: '01209-000', city: 'São Paulo' }] };
...
test('calculates cart subtotal and quantity from product prices', () => {
  const result = calculateCart([{ id: 'p1', price: 18 }, { id: 'p2', price: 32 }], { p1: 2, p2: 1 });
...
test('builds monthly revenue grouped by receiving account', () => {
  ...
  assert.deepEqual(report.byAccount, { 'asaas-main': 150, 'pix-secondary': 75 });
});
