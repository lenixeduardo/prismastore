...
test('simulador usa mensagens configuradas e não exige confirmação de maioridade', () => {
  assert.match(appSource, /messageValue\(state\.settings,/);
  assert.match(appSource, /chatbotMessages/);
  assert.doesNotMatch(appSource, /age-confirm/);
  assert.doesNotMatch(appSource, /18 anos|18\+/i);
});
...
test('simulador renderiza imagem do QR real e não usa placeholder quadriculado', () => {
  assert.match(appSource, /\/api\/payments\/demo-pix/);
  assert.match(appSource, /data:image\/png;base64/);
  assert.doesNotMatch(appSource, /<div class=\"qr-demo\"><\/div>/);
});
